...
void CControladora::verificarColisionHabilidad() {
    auto& habilidades = this->habilidades->getHabilidades();
    CJugador* jugador = this->jugador;

    for (auto it = habilidades.begin(); it != habilidades.end(); ) {
        CHabilidad* h = *it;
        if (h->getEstado() == EEstadoHabilidad::Visible &&
            jugador->getFila() == h->getFila() &&
            jugador->getColumna() == h->getColumna()) {

            switch (h->getTipoHabilidad()) {
            case 1: // Bomba adicional
                jugador->activarBombaAdicional();
                break;
            case 4: // Patines (aumentar velocidad)
                jugador->aumentarVelocidad();
                break;
            case 5: // Vida extra
                jugador->aumentarVidas(1);
                break;
            case 3: // Calavera (quita vidas)
                jugador->disminuirVida();
                break;
            }

            h->consumir();
            it = habilidades.erase(it);
        }
        else {
            ++it;
        }
...
void CControladora::crearHabilidades() {
    while (habilidades->getHabilidades().size() < 15) {
        ...
        habilidades->crearHabilidad(fila, columna);
    }
}
