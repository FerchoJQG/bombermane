...
CHabilidad::CHabilidad() {
    ...
    if (tipoHabilidad== 1) { // Bomba Adicional
        std::cout << "¡Bomba Adicional" << std::endl;
    }
}
...
const int CHabilidad::DURACION_SEGUNDOS = 15;
...
EEstadoHabilidad CHabilidad::getEstado() { return estado; }
int CHabilidad::getTipoHabilidad() { return tipoHabilidad; }
int CHabilidad::getFila() { return i; }
int CHabilidad::getColumna() { return j; }
bool CHabilidad::estaEnPosicion(int fila, int columna) { return (i == fila && j == columna); }
void CHabilidad::setPosicion(int fila, int columna) { i = fila; j = columna; ubicado = true; }
void CHabilidad::consumir() { estado = EEstadoHabilidad::Desaparecida; }

void CHabilidad::dibujar(Graphics^ g, Bitmap^ bmpHabilidad, const CanchaArray& cancha) {
    if (estado != EEstadoHabilidad::Visible) return;
...
void CHabilidad::animar() {
    if (estado != EEstadoHabilidad::Visible) return;
    switch (tipoHabilidad) {
    case 1: indiceX = 0; indiceY = (indiceY + 1) % 2; break;
    case 2: indiceX = 4; indiceY = (indiceY + 1) % 2; break;
    case 3: indiceX = 5; indiceY = (indiceY + 1) % 2; break;
    case 4: indiceX = 8; indiceY = (indiceY + 1) % 2; break;
    case 5: indiceX = 9; indiceY = (indiceY + 1) % 2; break;
    }
}
